import { siteConfig } from '@/lib/site-config';
import { ContactForm } from '@/components/ui/ContactForm';
import styles from './ContactSection.module.css';

function DetailIcon({ type }: { type: 'location' | 'phone' | 'email' | 'clock' }) {
  const icons = {
    location: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
        <circle cx="12" cy="10" r="3" />
      </svg>
    ),
    phone: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
      </svg>
    ),
    email: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
        <polyline points="22,6 12,13 2,6" />
      </svg>
    ),
    clock: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <circle cx="12" cy="12" r="10" />
        <polyline points="12 6 12 12 16 14" />
      </svg>
    ),
  };
  return <span className={styles.detailIcon}>{icons[type]}</span>;
}

export function ContactSection() {
  const { contact } = siteConfig;

  return (
    <section id="contacto" className={`section ${styles.section}`}>
      <div className="container">
        <h2 className="sectionTitle">{contact.title}</h2>
        <p className="sectionSubtitle">{contact.subtitle}</p>

        <div className={styles.grid}>
          <div className={styles.detailsCol}>
            {contact.details.map((detail) => (
              <div key={detail.label} className={`card ${styles.detailCard}`}>
                <DetailIcon type={detail.icon} />
                <div>
                  <span className={styles.detailLabel}>{detail.label}</span>
                  <p className={styles.detailValue}>{detail.value}</p>
                </div>
              </div>
            ))}
          </div>

          <div className={`card ${styles.formCard}`}>
            <ContactForm />
          </div>
        </div>
      </div>
    </section>
  );
}
